...
screen = width, height = windll.user32.GetSystemMetrics(
...
player_imgs = [pygame.image.load(
    IMGS_PATH + '/' + file).convert_alpha() for file in os.listdir(IMGS_PATH)]
...
def create_enemy():
    ...
    enemy = pygame.transform.scale(pygame.image.load(
        resource_path("enemy.png")).convert_alpha(), enemy_size)
    enemy_rect = pygame.Rect(width, random.randint(
        50, height-50), *enemy.get_size())
...
def create_bonus():
    ...
    bonus = pygame.transform.scale(pygame.image.load(
        resource_path("bonus.png")).convert_alpha(), bonus_size)
    bonus_rect = pygame.Rect(random.randint(
        40, width-40), 0, *bonus.get_size())
    bonus_speed = random.randint(2, 5)
    return [bonus, bonus_rect, bonus_speed]


bg = pygame.transform.scale(pygame.image.load(
    resource_path("background.png")).convert(), screen)
bgx = 0
bgx2 = bg.get_width()
bg_speed = 3
...
while Is_working:
    ...
    for enemy in enemies:
        ...
        if enemy[1].left + enemy[1].width < 0:
